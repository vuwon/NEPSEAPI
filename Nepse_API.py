...
try:
    ...
    selected_symbols = [
        'BANDIPUR', 'BHCL', 'BUNGAL', 'CREST', 'DHEL', 'SAGAR', 'HIMSTAR', 'JHAPA', 'NMIC', 'OMPL', 'SAIL', 'SANVI', 'SWASTIK', 'SYPNL', 'TTL', 'SABBL', 'HFIN', 'RSML', 'SOHL','BJHL','RLEL','SKHEL','SKHL'   
    ...
    with pd.ExcelWriter(combined_filename, engine='openpyxl') as writer:
        ...
        # Sheet: Highest volume stocks (descending) - now as the second sheet
        print('Available columns in DataFrame:', df.columns.tolist())
        volume_col = None
        for col in ['volume', 'qty', 'quantity', 'contractQuantity']:
            if col in df.columns:
                volume_col = col
                break
        if 'stockSymbol' in df.columns and volume_col:
            volume_df = df.groupby('stockSymbol', as_index=False)[volume_col].sum()
            volume_df = volume_df.sort_values(by=volume_col, ascending=False)
            volume_df.insert(0, 'S.N', range(1, len(volume_df) + 1))
            volume_df.to_excel(writer, sheet_name='Highest_Volume', index=False)
        else:
            print(f"Could not find 'stockSymbol' or a volume column (tried: 'volume', 'qty', 'quantity', 'contractQuantity') for highest volume sheet.")
