...
async function buildCommonStocks(sym, brokerNums, dfrom, dto){{
  ...
  try{{
    ...
    const brokerSet = new Set(brokerNums.map(String));
    const commonSyms = Object.entries(symBrkMap).filter(([sym, brkData])=>{{
      // Every selected broker must have bought at least 1 share
    ...
    `<table style="min-width:${{minW}}px;width:100%;border-collapse:collapse">
      <thead>
        <tr>
          ...
          <th rowspan="2" onclick="sortCs('totalBuy')" style="padding:6px 8px;min-width:70px">Total Buy ↕</th>
          ${{thBrk}}
        </tr>
        <tr>${{thSub}}</tr>
      </thead>
