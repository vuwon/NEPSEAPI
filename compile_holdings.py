"""
compile_holdings.py — NEPSE Holdings Compiler (Option 4: JSON-only storage)

HOW IT WORKS:
  - Reads today's Excel file(s) from ./data/ folder
  - Loads previous holdings from holdings_summary.json (if it exists)
  - Merges today's new data with historical aggregated data
  - Saves updated holdings_summary.json + index.html
  - Excel files are NEVER committed to GitHub — only JSON is stored

This means the JSON grows gradually each day but stays small
because it only stores aggregated non-zero holdings (not raw transactions).

Run locally : python compile_holdings.py
Run on GitHub Actions: same command
"""
...
def aggregate_one_file(df):
    """Aggregate one day's raw transactions into broker-level holdings."""
...
def load_previous_holdings():
    """
    Load previously stored holdings from holdings_summary.json.
    Returns a DataFrame of historical daily holdings, or empty DataFrame if none.
    """
    if not os.path.exists(OUTPUT_JSON):
        print("  No previous holdings_summary.json found — starting fresh.")
        return pd.DataFrame(), pd.DataFrame(columns=["broker","broker_name"]), pd.DataFrame(columns=["Stock Symbol","Security Name"])
    ...
    try:
        ...
        # Expand compact keys back to full column names
        df = pd.DataFrame([{
            "Date"           : r.get("d",""),
            "Stock Symbol"   : r.get("s",""),
            "Security Name"  : r.get("sn",""),
            "broker"         : int(r.get("b",0)),
            "broker_name"    : r.get("bn",""),
            "buy_qty"        : float(r.get("bq",0)),
            "buy_amt"        : float(r.get("ba",0)),   # stored in new format
            "total_sale_qty" : float(r.get("sq",0)),
            "ipo_sale_qty"   : float(r.get("iq",0)),
            "bulk_sale_qty"  : float(r.get("lq",0)),
            "bulk_sale_amt"  : float(r.get("la",0)),   # stored in new format
            "holding_qty"    : float(r.get("hq",0)),
            "avg_rate"       : float(r.get("ar",0)),
        } for r in records])

        # Extract broker names and security names lookups
        broker_names = df[["broker","broker_name"]].drop_duplicates()
        sec_names    = df[["Stock Symbol","Security Name"]].drop_duplicates()

        print(f"  Loaded {len(df):,} previous holdings from {OUTPUT_JSON}")
        return df, broker_names, sec_names

    except Exception as e:
        print(f"  WARNING: Could not load previous holdings: {e} — starting fresh.")
        return pd.DataFrame(), pd.DataFrame(columns=["broker","broker_name"]), pd.DataFrame(columns=["Stock Symbol","Security Name"])


def build_summary(h, broker_names_df, sec_names_df, all_symbols, all_dates):
    """Build compact JSON summary for dashboard."""

    # Attach names
    if not broker_names_df.empty:
        h = h.merge(broker_names_df, on="broker", how="left")
    if "broker_name" not in h.columns:
        h["broker_name"] = ""
    h["broker_name"] = h["broker_name"].fillna("")

    if not sec_names_df.empty:
        h = h.merge(sec_names_df, on="Stock Symbol", how="left")
    ...
    # Compact keys to reduce file size
    def compress(r):
        return {
            "d" : r.get("Date",""),
            ...
            "ba": round(float(r.get("buy_amt",0)),2),      # buy amount (needed for avg rate)
            "sq": int(r.get("total_sale_qty",0)),
            "iq": int(r.get("ipo_sale_qty",0)),
            "lq": int(r.get("bulk_sale_qty",0)),
            "la": round(float(r.get("bulk_sale_amt",0)),2), # bulk sale amount (needed for avg rate)
    ...
    return {
        ...
        "sy": sorted(all_symbols),
        "dt": sorted(all_dates),
        "tb": top,
        "h" : [compress(r) for r in recs.to_dict(orient="records")],
...
def main():
    print("="*60)
    print("NEPSE Holdings Compiler  (JSON-only mode)")
    print("="*60)
    os.makedirs(DATA_DIR, exist_ok=True)
    t0 = time.time()

    # ── Step 1: Load today's Excel files ─────────────────────────────────
    ...
    print(f"\nFound {len(files)} new Excel file(s):\n")
    new_frames   = []
    ...
    for i, fpath in enumerate(files, 1):
        ...
        new_symbols.update(df["Stock Symbol"].dropna().unique())
        new_dates.update(df["Date"].dropna().unique())
        del df
    ...
    today_h = pd.concat(new_frames, ignore_index=True)
    del new_frames
    print(f"\nToday's new positions: {len(today_h):,}")

    # ── Step 2: Load previous holdings from JSON ──────────────────────────
    print("\nLoading previous holdings...")
    prev_h, prev_brokers, prev_sec = load_previous_holdings()

    # Merge broker/sec name lookups
    if not prev_brokers.empty:
        broker_names = pd.concat([prev_brokers, broker_names]).drop_duplicates(subset=["broker"])
    if not prev_sec.empty:
        sec_names = pd.concat([prev_sec, sec_names]).drop_duplicates(subset=["Stock Symbol"])

    # ── Step 3: Combine today + history ───────────────────────────────────
    print("Merging with historical data...")
    if not prev_h.empty:
        # Drop columns that are only in prev_h (broker_name, Security Name — we re-attach later)
        prev_core = prev_h[["Date","Stock Symbol","broker","buy_qty","buy_amt",
                             "total_sale_qty","ipo_sale_qty","bulk_sale_qty","bulk_sale_amt",
                             "holding_qty"]].copy()
        # Remove today's dates from prev (re-process today fresh)
        prev_core = prev_core[~prev_core["Date"].isin(new_dates)]
        all_h = pd.concat([prev_core, today_h], ignore_index=True)
        all_symbols = new_symbols | set(prev_core["Stock Symbol"].dropna().unique())
        all_dates   = new_dates   | set(prev_core["Date"].dropna().unique())
    else:
        all_h       = today_h
        all_symbols = new_symbols
        all_dates   = new_dates

    # Re-aggregate in case same broker+symbol+date appears in multiple files
    grp = ["Date","Stock Symbol","broker"]
    all_h = (all_h.groupby(grp)
               ...
               .reset_index())
    all_h["holding_qty"] = all_h["buy_qty"] - all_h["bulk_sale_qty"]
    all_h["avg_rate"]    = ((all_h["buy_amt"] - all_h["bulk_sale_amt"]) / all_h["holding_qty"]).where(all_h["holding_qty"]>0, 0).round(2)
    all_h = all_h[all_h["holding_qty"] != 0]
    all_h = all_h.sort_values(["Stock Symbol","Date","holding_qty"],
                               ascending=[True,True,False]).reset_index(drop=True)

    nz = all_h[all_h["holding_qty"] != 0]
    print(f"\nFinal dataset:")
    print(f"  Holdings  : {len(nz):,} non-zero positions")
    print(f"  Symbols   : {sorted(all_symbols)}")
    print(f"  Brokers   : {all_h['broker'].nunique()}")
    print(f"  Dates     : {min(all_dates)} → {max(all_dates)}")
    print(f"  Date count: {len(all_dates)} trading days")

    # ── Step 4: Save JSON ──────────────────────────────────────────────────
    print(f"\nSaving {OUTPUT_JSON} ...")
    summary = build_summary(all_h.copy(), broker_names, sec_names, list(all_symbols), list(all_dates))
    with open(OUTPUT_JSON, "w") as f:
        json.dump(summary, f, separators=(',',':'), default=str)
    size_mb = os.path.getsize(OUTPUT_JSON)/1024/1024
    print(f"  Size: {size_mb:.2f} MB  ({len(summary['h']):,} records)")
    ...
    if os.environ.get("GITHUB_ACTIONS"):
        ...
        user = repo.split("/")[0]
        name = repo.split("/")[1]
        print(f"   Dashboard: https://{user}.github.io/{name}/")
    else:
        print("   Local: run  python -m http.server 8000  then open http://localhost:8000")


def write_html():
    """Lightweight index.html — loads holdings_summary.json via fetch (works on GitHub Pages)."""
    html = r"""<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
...
async function init(){
  try{
    ...
    document.getElementById('tbody-d').innerHTML=
      ...
      <small>Run <code>python compile_holdings.py</code> to generate holdings_summary.json</small>
      </div></td></tr>`;
  }
}
...
function renderD(){
  ...
  tb.innerHTML=sl.map(r=>{
    ...
    return`<tr>
      <td class="m">${r.d}</td><td class="sym">${r.s}</td>
      <td><span class="brk">${r.b}</span></td>
      <td class="bname">${r.bn||'—'}</td>
      <td class="m">${fmt(r.bq)}</td><td class="m">${fmt(r.sq)}</td>
...
function renderChart(sym){
  ...
  if(!sym){title.textContent='Select a stock symbol to see top brokers';
    chart.innerHTML=`<div class="empty">Use the Stock Symbol filter above, click Apply.</div>`;return;}
