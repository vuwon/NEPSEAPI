...
<header><div class="wrap"><div class="hdr">
  ...
  <div class="hdr-meta">DB updated: <b id="gen-at">—</b><br>Data range: <b id="date-range">—</b></div>
</div></div></header>
...
<div class="wrap" style="padding-top:20px;padding-bottom:40px">
  ...
  <!-- Filters — Symbol is required to load data -->
  <div class="fp">
    <div class="fg"><label>Stock Symbol ★</label>
      <input type="text" id="f-sym" placeholder="e.g. SAGAR, NABIL" style="text-transform:uppercase"></div>
    <div class="fg"><label>Broker #</label>
      <input type="text" id="f-brk" placeholder="e.g. 58"></div>
    <div class="fg"><label>Broker Name</label>
      <input type="text" id="f-bname" placeholder="e.g. Sunrise"></div>
    <div class="fg"><label>Date From</label>
      <input type="text" id="f-dfrom" placeholder="YYYY-MM-DD"></div>
    <div class="fg"><label>Date To</label>
      <input type="text" id="f-dto" placeholder="YYYY-MM-DD"></div>
    <div class="fg"><label>Min Holding</label>
      <input type="text" id="f-minq" placeholder="e.g. 500"></div>
  ...
  <div class="status-bar" id="status">Enter a stock symbol above and click Search to load data.</div>
  ...
  <div id="tab-daily">
    <div class="tw">
      <div class="th2"><span class="ttitle">Daily Holdings per Broker</span><span class="tcnt" id="cnt-d">—</span></div>
      <div class="tscroll"><table>
        <thead><tr>
          <th onclick="srtD('date')">Date ↕</th>
          <th>Symbol</th>
          <th onclick="srtD('broker')">Broker # ↕</th>
          <th>Broker Name</th>
          <th onclick="srtD('buy_qty')">Buy Qty ↕</th>
          <th onclick="srtD('total_sale_qty')">Sale Qty ↕</th>
          <th onclick="srtD('ipo_sale_qty')">IPO Sale ↕</th>
          <th onclick="srtD('bulk_sale_qty')">Bulk Sale ↕</th>
          <th onclick="srtD('holding_qty')">Net Holding ↕</th>
          <th onclick="srtD('avg_rate')">Avg Rate ↕</th>
        </tr></thead>
        <tbody id="tbody-d"><tr><td colspan="10"><div class="empty">Enter a symbol and click Search.</div></td></tr></tbody>
      </table></div>
      <div class="pag">
        <span id="pi-d">—</span>
        <div class="pbtns">
  ...
  <div id="tab-cumul" style="display:none">
    <div class="tw">
      <div class="th2"><span class="ttitle">Cumulative Net Holdings</span><span class="tcnt" id="cnt-c">—</span></div>
      <div class="tscroll"><table>
        <thead><tr>
          <th>Rank</th>
          <th>Symbol</th>
          <th onclick="srtC('broker')">Broker # ↕</th>
          <th>Broker Name</th>
          <th onclick="srtC('total_buy_qty')">Total Buy ↕</th>
          <th onclick="srtC('total_sale_qty')">Total Sale ↕</th>
          <th onclick="srtC('total_ipo_qty')">IPO Sale ↕</th>
          <th onclick="srtC('total_bulk_qty')">Bulk Sale ↕</th>
          <th onclick="srtC('net_holding')">Net Holding ↕</th>
          <th onclick="srtC('avg_rate')">Avg Rate ↕</th>
        </tr></thead>
        <tbody id="tbody-c"><tr><td colspan="10"><div class="empty">Enter a symbol and click Search.</div></td></tr></tbody>
      </table></div>
      <div class="pag">
        <span id="pi-c">—</span>
        <div class="pbtns">
  ...
  <div id="tab-topb" style="display:none">
    <div class="cw">
      <div class="ctitle" id="chart-title">Top 10 Brokers by Cumulative Net Holdings</div>
      <div class="barchart" id="barchart"><div class="empty">Enter a symbol and click Search.</div></div>
    </div>
  </div>
</div>
...
async function applyFilters() {{
  ...
  if(!sym) {{ setStatus('⚠ Please enter a stock symbol to search.', true); return; }}

  const brk   = document.getElementById('f-brk').value.trim();
  const bname  = document.getElementById('f-bname').value.trim().toLowerCase();
  const dfrom  = document.getElementById('f-dfrom').value.trim();
  const dto    = document.getElementById('f-dto').value.trim();
  const minq   = parseFloat(document.getElementById('f-minq').value)||null;
  const side   = document.getElementById('f-side').value;

  loading = true;
  setStatus(`<span>Loading data for <b>${{sym}}</b>…</span>`);
  showSpinner();

  try {{
    // ── Fetch daily holdings ──────────────────────────────────────
    ...
    if(minq!==null)  dq = dq.gte('holding_qty', side==='neg'?-999999999:minq);
    dq = dq.limit(5000);
    ...
    // Client-side broker name filter on daily (Supabase ilike on daily too)
    ...
    if(dates.length) {{
      ...
      document.getElementById('date-range').textContent = `${{sorted[0]}} → ${{sorted[sorted.length-1]}}`;
    }}
    document.getElementById('gen-at').textContent = new Date().toLocaleTimeString();

    pg.d=1; pg.c=1;
    renderD(); renderC(); renderChart(sym);
    setStatus(`<b>${{DAILY.length.toLocaleString()}}</b> daily positions · <b>${{CUMUL.length}}</b> broker cumulative positions for <b>${{sym}}</b>`);
...
function showSpinner() {{
  ['tbody-d','tbody-c'].forEach(id=>{{
    document.getElementById(id).innerHTML=`<tr><td colspan="10"><div class="loading"><div class="spinner"></div>Querying Supabase…</div></td></tr>`;
  }});
  document.getElementById('barchart').innerHTML=`<div class="loading"><div class="spinner"></div>Loading…</div>`;
}}

function resetFilters() {{
  ['f-sym','f-brk','f-bname','f-dfrom','f-dto','f-minq'].forEach(id=>document.getElementById(id).value='');
  document.getElementById('f-side').value='all';
  DAILY=[]; CUMUL=[]; FD=[]; FC=[];
  ['tbody-d','tbody-c'].forEach(id=>{{
    document.getElementById(id).innerHTML=`<tr><td colspan="10"><div class="empty">Enter a symbol and click Search.</div></td></tr>`;
  }});
  document.getElementById('barchart').innerHTML=`<div class="empty">Enter a symbol and click Search.</div>`;
  ['s-pos','s-brk','s-net','s-top'].forEach(id=>document.getElementById(id).textContent='—');
  setStatus('Enter a stock symbol above and click Search to load data.');
}}
...
function srtC(c){{if(cCol===c)cAsc=!cAsc;else{{cCol=c;cAsc=false;}}pg.c=1;renderC();}}
...
function renderD() {{
  ...
  if(!sl.length){{tb.innerHTML=`<tr><td colspan="10"><div class="empty">No results.</div></td></tr>`;return;}}
  tb.innerHTML=sl.map(r=>{{
    ...
    return`<tr>
      ...
      <td><span class="ipo">${{fmt(r.ipo_sale_qty)}}</span></td>
...
function showTab(name){{
  ...
  if(name==='topb') renderChart(document.getElementById('f-sym').value.toUpperCase());
}}
function chpg(dir,t){{pg[t]+=dir;if(t==='d')renderD();else renderC();window.scrollTo({{top:0,behavior:'smooth'}});}}

// Allow Enter key to trigger search
document.getElementById('f-sym').addEventListener('keydown', e=>{{ if(e.key==='Enter') applyFilters(); }});
</script>
</body>
</html>"""
