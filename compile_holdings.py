...
async function buildCommonStocks(sym, brokerNums, dfrom, dto){{
  ...
  document.getElementById('cs-table-wrap').innerHTML=
    '<div class="loading"><div class="spinner"></div>Loading stocks per broker…</div>';
  try{{
    // Fetch all holdings for selected brokers in date range — ALL symbols
    let all=[], off=0, lim=1000;
    while(true){{
      const {{data,error}}=await sb.from('holdings').select(
        'symbol,broker,broker_name,buy_qty,buy_amt,total_sale_qty,ipo_sale_qty,bulk_sale_qty,bulk_sale_amt,holding_qty,avg_rate'
    ...
    for(const r of all){{
      ...
      symBrkMap[s][b].buy_qty    +=(r.buy_qty||0);
      symBrkMap[s][b].buy_amt    +=(r.buy_amt||0);
      symBrkMap[s][b].sale_qty   +=(r.total_sale_qty||0);
      symBrkMap[s][b].ipo_qty    +=(r.ipo_sale_qty||0);
      symBrkMap[s][b].bulk_qty   +=(r.bulk_sale_qty||0);
      symBrkMap[s][b].bulk_amt   +=(r.bulk_sale_amt||0);
      symBrkMap[s][b].holding_qty+=(r.holding_qty||0);
    }}

    // Build flat CS_DATA sorted by total buy_qty desc
    CS_DATA = Object.entries(symBrkMap).map(([sym, brkData])=>{{
      const totalBuy = Object.values(brkData).reduce((s,b)=>s+b.buy_qty,0);
      const brokerCount = Object.keys(brkData).length;
      return {{ symbol:sym, totalBuy, brokerCount, brkData }};
...
function renderCsTable(data, brkMeta){{
  ...
  // Get sorted list of all broker numbers that appear
  const allBrokers=[...new Set(data.flatMap(r=>Object.keys(r.brkData).map(Number)))]
    .sort((a,b)=>a-b);
  ...
  const sorted=[...data].sort((a,b)=>{{
    ...
    if(csSortCol==='brokerCount') return csSortAsc?a.brokerCount-b.brokerCount:b.brokerCount-a.brokerCount;
    return csSortAsc?a.totalBuy-b.totalBuy:b.totalBuy-a.totalBuy;
  }});

  const medals=['🥇','🥈','🥉'];

  // Build thead — fixed cols + one group per broker
  let thBrk = allBrokers.map(b=>{{
    const name=(brkMeta[b]||'').split(' ')[0];
    return `<th colspan="4" style="text-align:center;background:var(--s2);border-left:2px solid var(--border2)">
      <span class="brk">${{b}}</span>
      <div style="font-size:10px;color:var(--muted);font-weight:400;margin-top:2px">${{name}}</div>
    </th>`;
  }}).join('');

  let thSub = allBrokers.map(()=>
    `<th onclick="sortCs('totalBuy')">Buy ↕</th>`+
    `<th>Sell</th>`+
    `<th>IPO</th>`+
    `<th>Holding<br><span style="font-weight:400;font-size:10px;color:var(--muted)">Rate</span></th>`
  ).join('');

  // Build rows
  const rows = sorted.map(r=>{{
    const rank=r.rank; const medal=medals[rank-1]||'#'+rank;
    let cells = allBrokers.map(b=>{{
      const d=r.brkData[b];
      if(!d||d.buy_qty===0&&d.sale_qty===0&&d.holding_qty===0){{
        return '<td colspan="4" style="color:var(--muted);text-align:center;border-left:2px solid var(--border2)">—</td>';
      }}
      ...
      const nhcls    = d.holding_qty>=0?'pos':'neg';
      return `<td style="border-left:2px solid var(--border2)">`+
        `<div class="m pos">${{fmt(d.buy_qty)}}</div>`+
        `<div style="color:var(--amber);font-size:10px">Rs ${{fmtf(buyRate)}}</div>`+
        `</td>`+
        `<td><div class="m neg">${{fmt(d.sale_qty)}}</div></td>`+
        `<td><span class="ipo">${{fmt(d.ipo_qty)}}</span></td>`+
        `<td><div class="m ${{nhcls}}">${{fmt(d.holding_qty)}}</div>`+
        `<div style="color:var(--amber);font-size:10px">Rs ${{fmtf(holdRate)}}</div></td>`;
    }}).join('');
    return `<tr>`+
      `<td class="m" style="color:var(--muted)">${{medal}}</td>`+
      `<td class="sym">${{r.symbol}}</td>`+
      `<td class="m pos">${{fmt(r.totalBuy)}}</td>`+
      `<td><span class="brk" style="font-size:10px">${{r.brokerCount}}</span></td>`+
      cells+
      `</tr>`;
  }}).join('');

  document.getElementById('cs-table-wrap').innerHTML=
    `<table style="min-width:${{400+allBrokers.length*200}}px">
      <thead>
        <tr>
          <th onclick="sortCs('rank')" rowspan="2"># ↕</th>
          <th onclick="sortCs('symbol')" rowspan="2">Symbol ↕</th>
          <th onclick="sortCs('totalBuy')" rowspan="2">Total Buy ↕</th>
          <th onclick="sortCs('brokerCount')" rowspan="2">Brokers</th>
          ${{thBrk}}
        </tr>
        <tr>${{thSub}}</tr>
      </thead>
