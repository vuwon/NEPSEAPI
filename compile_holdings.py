...
async function buildCommonStocks(sym, brokerNums, dfrom, dto){{
  ...
  try{{
    ...
    // Build CS_DATA — filter positive holdings only, sort by total net holding
    const commonSymList = commonSyms.map(([s])=>s);

    // Fetch LTP for common symbols from daily_volume (latest date)
    const ltpMap={{}};
    try{{
      const {{data:ltpData}}=await sb.from('daily_volume')
        .select('symbol,ltp,vwap')
        .in('symbol',commonSymList)
        .order('date',{{ascending:false}})
        .limit(commonSymList.length*2);
      if(ltpData) ltpData.forEach(r=>{{
        if(!ltpMap[r.symbol]) ltpMap[r.symbol]={{ltp:r.ltp||0,vwap:r.vwap||0}};
      }});
    }}catch(e){{console.warn('LTP fetch failed:',e);}}
    ...
    const allCsData = commonSyms.map(([sym, brkData])=>{{
      ...
      const ltp          = ltpMap[sym]?.ltp||0;
      const vwap         = ltpMap[sym]?.vwap||0;
      return {{ symbol:sym, totalHolding, totalBuy, ltp, vwap, brkData }};
    }})
    .filter(r=>r.totalHolding>0)          // only positive combined holdings
    .sort((a,b)=>b.totalHolding-a.totalHolding);

    if(!allCsData.length){{
      document.getElementById('cs-cnt').textContent='No common symbols with positive holdings';
      document.getElementById('cs-table-wrap').innerHTML=
        '<div class="empty">No common symbols where ALL selected brokers have positive net holdings.</div>';
      return;
    }}
    ...
    document.getElementById('cs-cnt').textContent=
      'Top '+CS_DATA.length+' of '+allCsData.length+' common symbols · sorted by net holding · '+brokerNums.length+' brokers · '+dfrom+' → '+dto;
    renderCsTable(CS_DATA, CS_BROKERS_META);
    ...
    `<table style="min-width:${{minW}}px;width:100%;border-collapse:collapse">
      <thead>
        <tr>
          ...
          <th rowspan="2" onclick="sortCs('ltp')" style="padding:6px 8px;min-width:80px">Market Vol ↕<div style="font-weight:400;font-size:9px;color:var(--muted)">Last traded price</div></th>
          ${{thBrk}}
        </tr>
        <tr>${{thSub}}</tr>
      </thead>
