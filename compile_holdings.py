...
<div class="wrap" style="padding-top:20px;padding-bottom:40px">
  ...
  <div id="tab-mkt" style="display:none">
    ...
    <div class="cw" style="margin-bottom:14px">
      ...
      <div style="display:flex;flex-wrap:wrap;gap:8px;padding:8px 0;align-items:center">
        <input type="text" id="accum-filter-sym" placeholder="Filter by symbol..."
          style="width:160px" oninput="filterAccumTable()"
          onkeydown="if(event.key==='Escape'){{this.value='';filterAccumTable();}}">
        <input type="text" id="accum-filter-brk" placeholder="Filter by broker #..."
          style="width:140px" oninput="filterAccumTable()"
          onkeydown="if(event.key==='Escape'){{this.value='';filterAccumTable();}}">
        <select id="accum-filter-pct" onchange="filterAccumTable()" style="width:150px">
          <option value="10">≥10% change</option>
          <option value="25">≥25% change</option>
          <option value="50">≥50% change</option>
          <option value="100">≥100% change</option>
        </select>
        <button class="btn btn-g" onclick="document.getElementById('accum-filter-sym').value='';
          document.getElementById('accum-filter-brk').value='';
          document.getElementById('accum-filter-pct').value='10';
          filterAccumTable()">Clear</button>
        <span id="accum-filter-cnt" style="font-size:11px;color:var(--muted)"></span>
      </div>
      <div style="display:flex;flex-wrap:wrap;gap:8px;padding:8px 0;align-items:center">
        <input type="text" id="accum-filter-sym" placeholder="Symbol..."
          style="width:120px" oninput="filterAccumTable()">
        <input type="text" id="accum-filter-brk" placeholder="Broker #..."
          style="width:110px" oninput="filterAccumTable()">
        <div class="fg" style="margin:0">
          <select id="accum-filter-date" onchange="filterAccumTable()" style="font-size:12px">
            <option value="">All dates</option>
          </select>
        </div>
        <div class="fg" style="margin:0">
          <select id="accum-filter-minhold" onchange="filterAccumTable()" style="font-size:12px">
            <option value="5000">Prev holding &gt; 5,000</option>
            <option value="0">All prev holdings</option>
            <option value="1000">Prev holding &gt; 1,000</option>
            <option value="10000">Prev holding &gt; 10,000</option>
            <option value="50000">Prev holding &gt; 50,000</option>
          </select>
        </div>
        <button class="btn btn-g" style="font-size:11px;padding:4px 10px"
          onclick="document.getElementById('accum-filter-sym').value='';
                   document.getElementById('accum-filter-brk').value='';
                   document.getElementById('accum-filter-date').value='';
...
function renderCsTable(data, brkMeta){{
  ...
  const thSub = allBrokers.map(()=>
    ...
    `<th style="font-size:9px">Hold<br>Rate</th>`
  ).join('');
  ...
  const rows=sorted.map(r=>{{
    ...
    return `<tr>
      ...
      <td class="m pos" style="padding:4px 8px;font-size:11px">${{fmt(r.totalBuy)}}</td>
      ${{cells}}
    </tr>`;
  }}).join('');

  // Compute approx width: 120px fixed + 160px per broker
  const minW = 120 + nBrk * 180;
    ...
    `<table style="min-width:${{minW}}px;width:100%;border-collapse:collapse">
      <thead>
        <tr>
          ...
          <th rowspan="2" onclick="sortCs('totalBuy')" style="padding:6px 8px;min-width:80px">Market Vol ↕<div style="font-weight:400;font-size:9px;color:var(--muted)">Total buy all brokers</div></th>
          ${{thBrk}}
        </tr>
        <tr>${{thSub}}</tr>
      </thead>
